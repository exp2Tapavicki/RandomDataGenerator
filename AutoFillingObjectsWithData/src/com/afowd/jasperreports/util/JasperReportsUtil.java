...
public class JasperReportsUtil {
	...
	/**
	 * create jasper report for object
	 * 
	 * @param object
	 ...
	 */
	public void getReport(Object object) throws ColumnBuilderException, JRException, ClassNotFoundException {
		Collection<Object> alData = new ArrayList<Object>();
		alData.add(object);
	}
	...
	private static void genereteJasperPrint(Collection<Object> alData) {
		try {
			getReport(alData.iterator().next(), headerStyle, detailTextStyle, detailNumberStyle, Boolean.FALSE, null, null);
	...
	private static AbstractColumn createColumn(String property, Class<?> type, String title, int width, Style headerStyle, Style detailStyle) throws ColumnBuilderException {
		AbstractColumn columnState = ColumnBuilder.getInstance().setColumnProperty(property, type.getName()).setTitle(title).setWidth(Integer.valueOf(width)).setStyle(detailStyle)
				.setHeaderStyle(headerStyle).build();
		return columnState;
	}

	private static void getReport(Object object, Style headerStyle, Style detailTextStyle, Style detailNumStyle, Boolean bInnerData, String sBeanName, String sParent) {
		try {
			...
			for (int j = 0; j < methods.length; j++) {
				...
				if (method.getName().startsWith(Const.STRING_GET)) {
					...
					if (!RandomUtil.listOfBasicTypes.contains(returnType.getName()) && !RandomUtil.listOfHashTypes.contains(returnType.getName())
						...
						getReport(method.invoke(object), headerStyle, detailTextStyle, detailNumStyle, Boolean.TRUE, (method.getName().substring(Const.THREE, method.getName().length())), object
