...
public class RandomDataUtil {
    ...
    private static void findOutBoundaries() {
        ...
        while (itrator.hasNext()) {
            ...
            if (hmBoundary.containsKey(sKey)) {
                boundary = hmBoundary.get(sKey);
            } else {
                boundary = new Boundary();
                hmBoundary.put(sKey, boundary);
            }
            String[] sClassPropertySplited = entry.getKey().split(Const.REGEX_MATCH_DOT);
                                ...
                                if (vectorParams != null) {
                                    if (vectorParams != null && !vectorParams.isEmpty()) {
                                        ...
                                        for (int k = Const.ZERO; k < getTempNumberOfData(sParameterType); k++) {
                                            ...
                                            hm.put(vector.get(Const.ZERO), vector.get(Const.ONE));
                                        }
                                ...
                                for (int k = Const.ZERO; k < getTempNumberOfData(sParameterType); k++) {
                                    ...
                                    for (Class<?> vectorParam : vectorParams) {
                                        if (RandomUtil.listOfBasicTypes.contains(vectorParam.getName())) {
                                            ...
                                            Object object = hmFilledClasses.get(vectorParam.getName()).get(RandomUtil.getRandomPrimitiveInt(Const.ZERO, getTempNumberOfData(vectorParam.getName()) - Const.ONE));
                                            vector.add(object);
                                        }
                                    }
                                    hm.put(vector.get(Const.ZERO), vector.get(Const.ONE));
    ...
    private static Object getRandomBasicObjectValue(String sClassName, boolean bAllowNulls, String sMethodName, String sSuperClassName) {
        ...
        switch (sClassName) {
            ...
            case BasicClassConstants.sEnum:
                Class<?> cl = getReturnParamFromMethod(sSuperClassName.replace(sPackageFromClass + Const.DOT_DELIMITER, Const.STRING_EMPTY), sMethodName.toLowerCase().substring(Const.THREE, sMethodName.length()));
